function [Trk,param,Obs_grap] = mot_generation_tracklet_HCF(rgbimg,Trk,Obs_grap,detections,param,Y_set,fr)
...
for i=1:length(Y_set(fr).child)
    ...
    if  length(find(ass_idx ~=0)) >= new_thr
        ct = ct + 1;
%-----------------------------------------------------------------------------------cf
         [Trk,param] = mot_tracklets_components_setup_HCF(rgbimg,Trk,detections,fr,ass_idx,param,[]);
%------------------------------------------------------------------------------------cf
        idx = [];
