...
%% All rights reserved.
% Y is only about detections originally.

% cur_det is the result in detection frame one---
cur_det = detections(1);
% for every object in frame one,setting their child = 0
...
for q=start_frame+1:end_frame
    ...
    asso_idx = [];
    % for every object in this frame
    for i=1:length(cur_det.x)
